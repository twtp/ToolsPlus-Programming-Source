...
namespace ListViewTools
{
    public static class ListViewTools
    {
        ...
        {
            string feed = System.IO.File.ReadAllText(tsvFile);
            ...
            foreach (string Row in feed.Split(new string[]{"\r\n"},StringSplitOptions.None))
            {
            ...
            if (newView.Items.Count > 0)
            {
                int columns = newView.Items[0].SubItems.Count;
                for (int x = 0; x < columns; x++)
                {
                    newView.Columns.Add(x.ToString());
                }
            }
            return newView;
        }
