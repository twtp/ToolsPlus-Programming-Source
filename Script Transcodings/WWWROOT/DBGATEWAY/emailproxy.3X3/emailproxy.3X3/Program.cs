...
namespace emailproxy._3X3
{
    ...
    /// <summary>
    /// to finish this script, in the Main method, take the processYAMLInput out from the top, and use it after the if/thens
    /// that determine if the calling computer and info are legit.
    /// </summary>
    ...
    class Program
    {
        ...
        public const string LAST_BUILD_DATE = "8/29/2014 - 11:57pm";
        ...
        public static void GETVERSION()
        {
            ...
            string VersionData = "Version " + VERSION + "\r\n" +
                ...
                "   This Script checks to see if the remote IP is an acceptable one.\r\n" +
                "   If so, it will take the 'POST' data in, aquire its variables, then\r\n" +
                "   send a confirmation email to the buyer of this particular order.\r\n" +
                "\r\n\r\n" +
                "   It gets tricky inside. There is a YAML code sent to this script to\r\n" +
                "   start. This code may or may not have multiple 'YAML' transactions,\r\n" +
                "   but we coded it to process them as well, just haven't debugged that.\r\n" +
                "   each transaction gets parsed in its own variable place. Then a\r\n" +
                "   confirmation email generated in either 'Text' or 'HTML' using\r\n" +
                "   said variables, then sent via the Emailer class to the receipient.\r\n" +
                "\r\n\r\n" +
                "   When debugging make sure to turn off the email list adding of\r\n" +
                "   customers emails or they will surely get upset. And obviously\r\n" +
                "   use your own email address for debugging. Smooth Sailin.\r\n\r\n" +
                "   This program is also script/service exclusive. Service\r\n" +
                "   however may not work as its untested.\r\n\r\n";
        ...
        static string ProcessYAMLinput(string[] args)
        {
            string PostData = GETPOST();
            //string CmdLineData = GETQUERY(args);
            //if (CmdLineData != "")
            //{
            //    PROCESSQUERY(CmdLineData);
            //}
            //PostData = args[0];
            orderDetailsList = YAMLtoOrderDetails(PostData);            
            
            string HTMLOUT = "<html><body style=\"font-family: sans-serif;\">\r\n";
            List<string> EmailList = new List<string>();
            foreach (OrderDetails od in orderDetailsList)
            {
                HTMLOUT += Get_EmailBody(od);
                //EmailList.Add(od.EmailAddress);
            ...
            //ImpersonationDemo.SendEmailImpersonated(Get_FromName(orderDetailsList[0].OrderNumber), HTMLOUT, false, EmailList);
            Emailer.SendEmailv2(Get_FromName(orderDetailsList[0].OrderNumber), HTMLOUT, false, true, EmailList);


            //Emailer.SendEmail("Subject: Confirmation Order\n", HTMLOUT, false, EmailList);
            Console.WriteLine("Status: 200 OK\nContent-type: text/plain\n\n200 OK");
        ...
        static void Main(string[] args)
        {
            ...
            string PostData = ProcessYAMLinput(args);
            Environment.Exit(0);
            ...
            if (PostData.Length > 0)
            {
                ...
                //changed out for code above
                ProcessYAMLinput(args);
            }
            else
            {
                Console.WriteLine("Status: 400 Bad Request\nContent-type: text/plain\n\nPOST Data Missing.");
